...
StandardRenderer.prototype._install = function (packages) {
    ...
    mout.object.forOwn(packages, function (pkg) {
        ...
        // List only 1 level deep bowerDependencies
        mout.object.forOwn(pkg.bowerDependencies, function (dependency) {
            dependency.bowerDependencies = {};
        });
...
StandardRenderer.prototype._tree2archy = function (node) {
    var bowerDependencies = mout.object.values(node.bowerDependencies);
    ...
    if (!bowerDependencies.length) {
        return label;
    }

    return {
        label: label,
        nodes: mout.object.values(bowerDependencies).map(this._tree2archy, this)
    };
};
