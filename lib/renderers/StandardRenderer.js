...
StandardRenderer.prototype.error = function (err) {
    ...
    if (this._config.verbose || !err.code || err.errno) {
        ...
        this._write("Bower version :" + pkg.version + '\n');
        this._write("Node version :" + process.versions.node + '\n');
        this._write("System info :" + os.platfrom() + '\n' );
        if (cmd.isWin){
          cmd('git', ['--version'])
          .spread(function (stdout) {
            this._write("git version :" + stdout.toString());
          });
        } else {
          exec('git --version',function(err,stdout,stderr){
            this._write("git version :" + stdout);
          });
        }
    }
};
