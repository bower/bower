...
function createInstance(decEndpoint, config, logger, registryClient) {
    ...
    //Calculate dependencies from one of the parents, not from the cwd
    var parentSource = config.cwd;
    if (decEndpoint.dependants && decEndpoint.dependants.length > 0){
       parentSource = path.resolve(config.cwd, decEndpoint.dependants[0].source);
    } 

    return getConstructor(decEndpoint.source, config, registryClient, parentSource)
...
function getConstructor(source, config, registryClient, parentSource) {
    var absolutePath,
        promise;
    ...
    absolutePath = path.resolve(parentSource, source);
