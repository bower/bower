...
function getConstructor(source, config, registryClient) {
    ...
    // Bzr case: bzr, bzr+ssh
    //           lp (launchpad)
    if (/^bzr(\+ssh)?:\/\//i.test(source) || (/^lp:/.test(source))) {
        return Q.fcall(function () {
            return [resolvers.Bzr, source];
        });
    }
    ...
    if (/^\.\.?[\/\\]/.test(source) || /^~\//.test(source) || path.normalize(source).replace(/[\/\\]+$/, '') === absolutePath) {
        ...
        .fail(function () {
            return Q.nfcall(fs.stat, path.join(absolutePath, '.bzr'))
            .then(function (stats) {
                if (stats.isDirectory()) {
                    return function () {
                        return Q.resolve([resolvers.Bzr, absolutePath]);
                    };
                }

                throw new Error('Not a Bazaar repository');
            });
        })
