...
GitHubResolver.prototype._checkout = function () {
    ...
    .progress(function (state) {
        ...
        if (state.retry) {
            ...
            that._logger.debug('error', state.error.code + ' ' + state.error.message);
    ...
    .then(function () {
        ...
        .fail(function (err) {
            msg =  'Decompression of ' + path.basename(file) + ' failed with ' + err.code + ', ';
            msg += 'trying with git..';
            that._logger.debug('error', err.code + ' ' + err.message);
            that._logger.warn('retry', msg);
            return GitRemoteResolver.prototype._checkout.call(that);
        });
    }, function (err) {
        msg = 'Download of ' + tarballUrl + ' failed with ' + err.code + ', ';
        msg += 'trying with git..';
        that._logger.debug('error', err.code + ' ' + err.message);
        that._logger.warn('retry', msg);
        return GitRemoteResolver.prototype._checkout.call(that);
