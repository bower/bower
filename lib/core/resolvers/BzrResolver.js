...
BzrResolver.prototype._findResolution = function (target) {
    ...
    if ((/^revno:-?\d+/).test(target)) {
        target = target.split(':');

        this._resolution = { type: 'revno', commit: target[1] };
    ...
    if (semver.validRange(target)) {
        ...
        .then(function (versions) {
            ...
            return Q.resolve(self.tags(that._source))
            .then(function (tags) {
...
BzrResolver.prototype._cleanup = function () {
    var bzrFolder = path.join(this._tempDir, '.bzr');

    return Q.nfcall(rimraf, bzrFolder);
};
