...
Resolver.prototype._savePkgMeta = function (meta) {
    ...
    // check package validity amd notify user
    var requiredAttrs = ['main', 'ignore'];
    var missingAttrs = [];
    ...
    if (missingAttrs.length === 1) {
        this._logger.log('warn', 'invalid-package', 'bower.json is missing recommended tag: ' + missingAttrs[0]);
    } else if (missingAttrs.length > 1) {
        this._logger.log('warn', 'invalid-package', 'bower.json is missing recommended tags: ' + missingAttrs.join(', '));
    }
