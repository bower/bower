...
Project.prototype.install = function (decEndpoints, options) {
    ...
    .spread(function (json, tree) {
        ...
        that.walkTree(tree, function (node, name) {
            ...
            // Ignore linked bowerDependencies because it's too complex to parse them
        ...
        if (that._options.save || that._options.saveDev) {
            ...
            decEndpoints.forEach(function (decEndpoint) {
                ...
                if (that._options.save) {
                    that._json.bowerDependencies = mout.object.mixIn(that._json.bowerDependencies || {}, jsonEndpoint);
                }

                if (that._options.saveDev) {
                    that._json.devbowerDependencies = mout.object.mixIn(that._json.devbowerDependencies || {}, jsonEndpoint);
                }
            });
        }

        // Save JSON, might contain changes to bowerDependencies and resolutions
...
Project.prototype.walkTree = function (node, fn, onlyOnce) {
    ...
    var queue = mout.object.values(node.bowerDependencies);
    ...
    while (queue.length) {
        ...
        // Add bowerDependencies to the queue
        bowerDependencies = mout.object.values(node.bowerDependencies);

        // If onlyOnce was true, do not add if already traversed
        if (onlyOnce) {
            bowerDependencies = bowerDependencies.filter(function (dependency) {
            ...
            onlyOnce.push.apply(onlyOnce, bowerDependencies);
        }

        queue.unshift.apply(queue, bowerDependencies);
    }
};
...
Project.prototype.saveJson = function (forceCreate) {
    ...
    if (!this._jsonFile && !forceCreate) {
        this._logger.warn('no-json', 'No package.json file to save to, use bower init to create one');
        return Q.resolve();
    }

    file = this._jsonFile || path.join(this._config.cwd, 'package.json');
...
Project.prototype._analyse = function () {
    ...
    .spread(function (json, installed, links) {
        ...
        // Mix direct extraneous as bowerDependencies
        // (bowerDependencies installed without --save/--save-dev)
        jsonCopy.bowerDependencies = jsonCopy.bowerDependencies || {};
        jsonCopy.devbowerDependencies = jsonCopy.devbowerDependencies || {};
        mout.object.forOwn(installed, function (decEndpoint, key) {
            var pkgMeta = decEndpoint.pkgMeta;
            var isSaved = jsonCopy.bowerDependencies[key] || jsonCopy.devbowerDependencies[key];

            // The _direct propery is saved by the manager when .newly is specified
            if (!isSaved && pkgMeta._direct) {
                decEndpoint.extraneous = true;
                jsonCopy.bowerDependencies[key] = (pkgMeta._originalSource || pkgMeta._source) + '#' + pkgMeta._target;
            }
        });

        // Restore the original bowerDependencies cross-references,
        // that is, the parent-child relationships
        this._restoreNode(root, flattened, 'bowerDependencies');
        // Do the same for the dev bowerDependencies
        if (!this._options.production) {
            this._restoreNode(root, flattened, 'devbowerDependencies');
        }
        ...
        mout.object.forOwn(flattened, function (decEndpoint, name) {
            if (!decEndpoint.dependants) {
                decEndpoint.extraneous = true;
                this._restoreNode(decEndpoint, flattened, 'bowerDependencies');
                // Note that it has no dependants, just bowerDependencies!
                root.bowerDependencies[name] = decEndpoint;
            }
        }, this);
...
Project.prototype._readJson = function () {
    ...
    .spread(function (json, deprecated, assumed) {
        ...
        if (!assumed) {
            that._jsonFile = path.join(that._config.cwd, deprecated ? deprecated : 'package.json');
        }
...
Project.prototype._readInstalled = function () {
    ...
    return this._installed = Q.nfcall(glob, '*/.package.json', {
        cwd: componentsDir,
        dot: true
    })
...
Project.prototype._removePackages = function (packages) {
    ...
    mout.object.forOwn(packages, function (dir, name) {
        ...
        if (that._options.save && that._json.bowerDependencies) {
            promise = promise
            .then(function () {
                delete that._json.bowerDependencies[name];
            });
        }

        if (that._options.saveDev && that._json.devbowerDependencies) {
            promise = promise
            .then(function () {
                delete that._json.devbowerDependencies[name];
            });
        }
...
Project.prototype._restoreNode = function (node, flattened, jsonKey) {
    ...
    node.bowerDependencies = node.bowerDependencies || {};
    ...
    deps = mout.object.filter(node.pkgMeta[jsonKey], function (value, key) {
        return !node.bowerDependencies[key];
    });
    ...
    mout.object.forOwn(deps, function (value, key) {
        ...
        // This might change in the future: #673
            ...
            // We only do this for direct root bowerDependencies that are compatible
