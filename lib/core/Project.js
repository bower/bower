...
Project.prototype.install = function (decEndpoints, options, config) {
    ...
    .then(function (installed) {
        ...
        if (that._options.save || that._options.saveDev || that._options.saveExact || that._config.save || that._config.saveExact) {
            ...
            decEndpoints.forEach(function (decEndpoint) {
                var depKey = that._options.saveDev ? 'devDependencies' : 'dependencies';
                var depList, jsonEndpoint;
                ...
                depList = mout.object.mixIn(that._json[depKey] || {}, jsonEndpoint);

                if (that._config.sortDependencies) {
                    depList = sortObject(depList);
                }

                that._json[depKey] = depList;
            });
        }
