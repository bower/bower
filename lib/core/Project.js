...
Project.prototype._readJson = function () {
    ...
    // Set assume data
    assume = {};
    // Default name to the package.json name or the directory name or root
    if (fs.existsSync(path.join(this._config.cwd, 'package.json'))) {
      ...
      assume.name = npm.name;
    }
    assume.name = assume.name || path.basename(this._config.cwd) || 'root';

    // Read local json
    return this._json = readJson(this._config.cwd, { assume: assume })
