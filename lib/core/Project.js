...
var cache = require('../commands/cache/clean');
...
Project.prototype.reset = function(){
    ...
    return this._analyse()
        .spread(function(json, tree, flattened){
            that.walkTree(tree, function (node, nodeName){
                // Create an array of currently installed packages
                componenets.push(nodeName);
            });
        })
        .then(function(){
            // Call uninstall method with the built components array
            return that.uninstall(componenets);
        })
        .then(function(){
            // Clear the cache
            return cache.line(that._logger);
        })
        .then(function(){
            // Grab latest version of packages and reinstall them
            return that.update();
        });
};
