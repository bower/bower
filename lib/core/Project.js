...
Project.prototype._readJson = function () {
    var defaults, npm, that = this;

    if (this._json) {
        return Q.resolve(this._json);
    }

    // Object is passed to `readJson` and assumed in place of missing bower.json
    defaults = {};

    // Check if `package.json` exists
    if (fs.existsSync(path.join(this._config.cwd, 'package.json'))) {
        // Read the package.json file
        npm = JSON.parse(fs.readFileSync(path.join(this._config.cwd, 'package.json')));

        // Det default values form package.json
        defaults.name        = npm.name;
    ...
    // Ensure defaults always has a name
    defaults.name = defaults.name || path.basename(this._config.cwd) || 'root';

    // Read local json
    return this._json = readJson(this._config.cwd, { assume: defaults })
