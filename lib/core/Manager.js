...
Manager.prototype.configure = function (setup) {
    ...
    this._suppliedDependencies = setup.suppliedDependencies;
...
Manager.prototype._fetch = function (decEndpoint) {
    ...
    // Check if supplied dependency
    if (this._isSuppliedDependency(name)) {
        this._logger.action('skipped', name, this.toData(decEndpoint));
        return;
    }
...
Manager.prototype._isSuppliedDependency = function (name) {
    if (this._suppliedDependencies[name]) {
        return true;
    }

    // do regex search
    var wasSuppliedDependency = false;
    mout.object.forOwn(this._suppliedDependencies, function (value, key) {
        if (wasSuppliedDependency) { return; }
        if (typeof value !== 'string') { return; }

        var regex = new RegExp(value, 'i');
        if (regex.test(name)) {
            wasSuppliedDependency = true;
        }
    });

    return wasSuppliedDependency;
};
