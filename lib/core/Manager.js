...
Manager.prototype.toData = function (decEndpoint, extraKeys) {
    ...
    if (decEndpoint.bowerDependencies) {
        data.bowerDependencies = {};

        // Call recursively for each dependency but ordered
        // by dependency names
        names = Object.keys(decEndpoint.bowerDependencies).sort();
        names.forEach(function (name) {
            data.bowerDependencies[name] = this.toData(decEndpoint.bowerDependencies[name], extraKeys);
        }, this);
    }
...
Manager.prototype._onFetchSuccess = function (decEndpoint, canonicalDir, pkgMeta, isTargetable) {
    ...
    // Parse bowerDependencies
    this._parsebowerDependencies(decEndpoint, pkgMeta, 'bowerDependencies');
...
Manager.prototype._parsebowerDependencies = function (decEndpoint, pkgMeta, jsonKey) {
    decEndpoint.bowerDependencies = decEndpoint.bowerDependencies || {};

    // Parse package bowerDependencies
    mout.object.forOwn(pkgMeta[jsonKey], function (value, key) {
        ...
        if (resolved) {
            ...
            if (compatible) {
                decEndpoint.bowerDependencies[key] = compatible;
            ...
            if (compatible) {
                ...
                childDecEndpoint.bowerDependencies = compatible.bowerDependencies;
        ...
        if (fetching) {
            ...
            if (compatible) {
                compatible.promise
                .then(function () {
                    this._parsebowerDependencies(decEndpoint, pkgMeta, jsonKey);
                }.bind(this));
                return;
            }
        }
        ...
        decEndpoint.bowerDependencies[key] = childDecEndpoint;
        childDecEndpoint.dependants = [decEndpoint];
