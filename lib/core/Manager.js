...
Manager.prototype._electSuitable = function (name, semvers, nonSemvers) {
    ...
    choices = dataPicks.map(function (pick, index) {
      return {
        value: picks[index],
        name: template.render('std/conflict-choice.std', pick)
      };
    });
    ...
    inquirer.prompt([
      {
        name: 'choice',
        type: 'list',
        message:  template.render('std/conflict-question.std', { name: name }),
        choices: choices
      },
      {
        name: 'save',
        type: 'confirm',
        message: 'Save change to bower.json',
        default: true
      }],
      deferred.resolve);
    return deferred.promise
    .then(function (results) {
        var pick = results.choice;
        var resolution;
