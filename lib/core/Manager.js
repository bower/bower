...
Manager.prototype._parseDependencies = function (decEndpoint, pkgMeta) {
    ...
    mout.object.forOwn(pkgMeta.dependencies, function (value, key) {
        ...
        //Adjust dependency source if relative, considering its own configuration, if any
        if (/^(\.|\.\.)\//g.test(childDecEndpoint.source)) {
            
            var absPath = path.resolve(pkgMeta._source, childDecEndpoint.source);

            //try get configuration for package
            var depConfg = configuration.readCachedConfig(absPath);
            
            //recalculate if cwd differ
            if (absPath !== depConfg.cwd)
                absPath = path.resolve(pkgMeta._source, depConfg.cwd);

            childDecEndpoint.source = absPath;
