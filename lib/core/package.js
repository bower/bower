...
Package.prototype.extract = function () {
  var package = this;
  var file = path.join(package.path, 'index' + package.assetType);
  template('action', { name: 'extracting', shizzle: file }).on('data', package.emit.bind(package, 'data'));

  /* See https://github.com/twitter/bower/issues/314 why 
   * streams aren't currently being used */
  fs.readFile(file, function(err, data) {
    if (err) {
      package.emit.bind(package, 'error');
      return;
    }

    var unzipFunction = package.assetType === '.zip' ? unzip : tar;
    var archived = unzipFunction.Extract({ path: package.path });

    archived.on('error', package.emit.bind(package, 'error'))
    archived.on('close', function () {
      // Delete zip
      fs.unlink(file, function (err) {
        if (err) return package.emit('error', err);

        // If we extracted only a folder, move all the files within it to the original path
        fs.readdir(package.path, function (err, files) {
          if (err) return package.emit('error', err);

          if (files.length !== 1) return package.emit('extract');

          var dir = path.join(package.path, files[0]);
          fs.stat(dir, function (err, stat) {
            if (err) return package.emit('error', err);
            if (!stat.isDirectory()) return package.emit('extract');

            fs.readdir(dir, function (err, files) {
              if (err) return package.emit('error', err);

              async.forEachSeries(files, function (file, next) {
                fs.rename(path.join(dir, file), path.join(package.path, file), next);
              }.bind(package), function (err) {
                if (err) return package.emit('error');

                fs.rmdir(dir, function (err) {
                  if (err) return package.emit('error');
                  package.emit('extract');
                });
              });
            });
          });
