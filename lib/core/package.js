...
Package.prototype.download = function () {
  this.assets = _.isArray(this.assetUrl) ? this.assetUrl : [this.assetUrl];
  ...
  tmp.dir({
    ...
    async.forEach(this.assets, function (asset, next) {
      var src;

      template('action', { name: 'downloading', shizzle: asset })
        .on('data', this.emit.bind(this, 'data'));
      ...
      req.get(src, function (res) {
        ...
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          template('action', { name: 'redirect detected', shizzle: res.headers.location})
            .on('data', this.emit.bind(this, 'data'));
          // this.assets.push(res.headers.location);
          redAssets.push(res.headers.location);
          return next();
        }

        // Detect not OK status codes
        if (res.statusCode < 200 || res.statusCode >= 300) {
          return this.emit('error', new Error(res.statusCode + ' status code for ' + asset));
        }

        var file = fs.createWriteStream(path.join((this.path = tmpPath), (this.assets && this.assets.length > 1 ? path.basename(asset) : 'index' + path.extname(asset))));
      ...
      if (redAssets.length) {
        this.assetUrl = redAssets;
        this.download();
      } else this.once('loadJSON', this.saveUnit).loadJSON();
    }.bind(this));
  }.bind(this));
};
