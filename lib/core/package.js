  ...
  tmp.dir({
    ...
    req.get(src, function (res) {
      ...
      if(res.headers['content-disposition']){
        matches = res.headers['content-disposition'].match(/filename=(.+)/);
        if(matches && matches[1]){
          this.assetType = path.extname(matches[1]);
          if(matches[1].indexOf('.tar.gz')!==-1){
            return this.emit('error', new Error('tar.gz files not yet supported, please use zip or vanilla tar.'));
          }
        }
      }
