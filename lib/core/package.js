...
Package.prototype.download = function () {
  ...
  tmp.dir({
    ...
    async.forEachSeries(this.assets, function (asset, next) {
      ...
      this.assetUrl = asset;
      this.assetType = path.extname(asset);
      ...
      req.get(src, function (res) {
        ...
        var file = fs.createWriteStream(path.join((this.path = tmpPath), (this.assets && this.assets.length > 1 ? path.basename(this.assetUrl) : 'index' + this.assetType)));
