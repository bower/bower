...
var JSON_FILENAME = 'component.json';
...
Package.prototype.cleanUpLocal = function () {
  ...
  fs.writeFile(path.join(this.localPath, JSON_FILENAME), jsonStr);
  if (this.gitUrl || this.gitPath) fs.writeFile(path.join(this.gitPath, JSON_FILENAME), jsonStr);
...
Package.prototype.loadJSON = function () {
  ...
  var jsonFile = path.join(this.path, JSON_FILENAME);
  fileExists(jsonFile, function (exists) {
    ...
    readJSON(jsonFile, function (err, json) {
      if (err) {
        err.details = 'There was an error while reading the ' + JSON_FILENAME;
      ...
      if (this.tag && (cleanedTag = semver.clean(this.tag)) && cleanedTag !== this.version) {
        ...
        if (!this.unitWork.retrieve('mismatch#' + this.name + '_' + cleanedTag)) {
          template('warning-mismatch', { name: this.name, json: JSON_FILENAME, tag: cleanedTag, version: this.version || 'N/A' })
            .on('data', this.emit.bind(this, 'data'));
