    ...
    if (this.tag) {
      ...
      spawn('git', [ 'checkout', this.tag, '-f'], { cwd: this.path }, this).on('close', function () {
        ...
        spawn('git', ['clean', '-f', '-d'], { cwd: this.path }, this).on('close', function () {
          // first check for a .gitmodules file
          fileExists(path.join(this.path, '.gitmodules'), function (exists) {
            if (!exists) {
              this.emit('checkout');
              this.loadJSON();
              return;
            ...
            spawn('git', ['submodule', 'init'], { cwd: this.path }, this).on('close', function () {
              // Update submodules
              spawn('git', ['submodule', 'update'], { cwd: this.path }, this).on('close', function () {
                this.emit('checkout');
                this.loadJSON();
              }.bind(this));
            }.bind(this));
