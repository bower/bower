...
Package.prototype.download = function () {
  ...
  tmp.dir({
    ...
    var req  = config.proxy ? request.defaults({ proxy: config.proxy }) : request;
    req.get(this.assetUrl, function (err, res, body) {
      if (err) return this.emit('error', err);

      this.assetUrl = res.request.uri.href;
      fs.writeFile(path.join((this.path = tmpPath), 'index' + this.assetType), body, function (err) {
        if (err) return this.emit('error', err);
