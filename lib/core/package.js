...
Package.prototype.install = function () {
  ...
  isRepo(localPath, function (is) {
    if (is) {
      var err = new Error('Local path (%s) is a local repository', localPath);
...
Package.prototype.getPublicPackage = function () {
  
  var packages = {};
  this.getDeepDependencies().forEach(function(pkg){
    packages[pkg.name] = packages[pkg.name] || [];
    packages[pkg.name].push(pkg.getPublicPackage());
  });
  
  // Always return the same publicPackage instance so we can still use === to test package equality externally.
  this.publicPackage = this.publicPackage || {};
  return _.extend(
    this.publicPackage,
    _.pick(this, 'name', 'localPath', 'json', 'version', 'tag', 'installed'),
    { 
