...
Package.prototype.download = function () {
  ...
  tmp.dir({
    ...
    req.get(src, function (res) {
      ...
      var content_disposition = res.headers['content-disposition'], matches = {};
      if(content_disposition){
        matches = content_disposition.match(/filename=(.+)/);
        if(matches && matches[1]){
          this.assetType = path.extname(matches[1]);
        }
      }
      ...
      res.on('end', function () {
        ...
        if (this.assetType === '.gz' && matches[1].search(/\.tar\.gz$/) !== -1) this.emit('warn','tar.gz package type not yet supported');
        next();
      }.bind(this));
