...
Package.prototype.versions = function () {
  if (versionCache[this.gitUrl]) {
    this.emit('versions', versionCache[this.gitUrl]);
  }
  var cp;

  if (this.gitUrl) {
    cp = git(['ls-remote', '-t', this.gitUrl], {}, this);
  } else {
    cp = git(['ls-remote', '-t'], { cwd: this.path }, this);
  }
  ...
  cp.on('close', function (code) {
    ...
    versions = versions.map(function (line) {
      return line.replace(/[a-f0-9]{40}\srefs\/tags\//, '');
    });
    ...
    if (versions.length) {
      ...
      return this.emit('versions', versions);
    }
