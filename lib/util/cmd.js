...
function getWindowsCommand(command) {
    var extension = path.extname(command).toLowerCase();
    ...
    // Use which to retrieve the full command, which puts the
    // extension in the end
    try {
        return winWhichCache[command] = which.sync(command);
    } catch (err) {
        return winWhichCache[command] = null;
    }
}
...
function cmd(command, args, options) {
    ...
    if (isWin) {
        command = getWindowsCommand(command) || command;
    }
