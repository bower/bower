...
function createLink(src, dst, type, isRelative) {
    ...
    .then(function (stat) {
        ...
        var _src = isRelative && !isWin ? path.relative(path.dirname(dst), src) : path.resolve(src);

        return Q.nfcall(fs.symlink, _src, dst, type)
        .fail(function (err) {
            ...
            return Q.nfcall(fs.symlink, _src, dst, 'junction')
            .fail(function (err) {
