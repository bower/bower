...
function convertGitUrl(url, logger) {
    var newUrl;

    if (url && url.match('^[^/]+/[^/]+$')) {
        var cmd = 'git remote -v | grep fetch | cut -f 2 | cut -d " " -f 1';
        var stdout = '' + cp.execSync(cmd);

        //caverse from: git@github.com:user/example.git
        //caverse to:   git://github.com/user/example.git
        if (stdout) {
            var stdoutArray = stdout.split(':');
            var gitAddress = stdoutArray.length? stdoutArray[0].replace('@', '://'): '';

            newUrl = gitAddress + '/' + url + '.git';
