...
function register(name, url, config) {
    ...
    process.nextTick(function () {
        ...
        .spread(function (canonicalDir, pkgMeta) {
            ...
            // check some basics of the bower.json to make sure it is not missing anything important
            var deferred, missingIgnore = !pkgMeta.ignore, missingMain = !pkgMeta.main;
            if (missingIgnore || missingMain) {
                var msg = 'This package is missing the recommended tag(s): ';
                if (missingIgnore) {
                    msg += 'ignore';
                    if (missingMain) {
                        msg += ' and ';
                ...
                msg += '.\nTo ensure happy users, it is strongly recommended to fix this before publishing. Continue?';
                deferred = Q.nfcall(logger.prompt.bind(logger), {
                    type: 'confirm',
                    message: msg,
                    default: false
                });
            } else {
               // if all looks good, OK to immediately resolve
                deferred = Q.defer();
