...
function setDefaults(config, json) {
    ...
    // Grab values from package.json if it exists
    if (fs.existsSync(path.join(config.cwd, 'package.json'))) {
        npm = JSON.parse(fs.readFileSync(path.join(config.cwd, 'package.json')));
    }

    // Name
    if (!json.name) {
        json.name = npm.name || path.basename(config.cwd);
    }
    ...
    if (!json.main) {
        ...
        if (npm.main) {
            json.main = npm.main;
        } else if (fs.existsSync(path.join(config.cwd, 'index.js'))) {
            json.main = 'index.js';
    ...
    if (!json.authors) {
        if(npm.author) {
            // Use author from package.json
            json.authors = npm.author;
        } else {
            promise = promise.then(function () {
    ...
    // License
    if (!json.license && npm.license) {
        json.license = npm.license;
    }

    // Keywords
    if (!json.keywords && npm.keywords) {
        json.keywords = npm.keywords;
    }

    // Description
    if (!json.description && npm.description) {
        json.description = npm.description;
    }
