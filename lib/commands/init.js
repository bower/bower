...
function init(config) {
    ...
    .spread(setIgnore)
    // Set bowerDependencies based on the response
    .spread(setbowerDependencies.bind(null, project))
    // All done!
...
function promptUser(logger, json) {
    var questions = [
        ...
        },
        {
            ...
            'message': 'set currently installed components as bowerDependencies?',
            'default': !mout.object.size(json.bowerDependencies) && !mout.object.size(json.devbowerDependencies),
            'type': 'confirm'
        },
        {
            'name': 'ignore',
...
function setbowerDependencies(project, json, answers) {
    if (answers.bowerDependencies) {
        return project.getTree()
        .spread(function (tree, flattened, extraneous) {
            if (extraneous.length) {
                json.bowerDependencies = {};

                // Add extraneous as bowerDependencies
                extraneous.forEach(function (extra) {
                    ...
                    mout.object.mixIn(json.bowerDependencies, jsonEndpoint);
                });
            }
