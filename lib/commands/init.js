...
function init(config) {
    ...
    // All done!
    .then(saveJson.bind(null, project))
    .then(function (json) {
...
function readJson(project, logger) {
    return project.getJson(false)
    .then(function (json) {
        if (json) {
            logger.warn('existent', 'The existent json file will be used and filled in');
        } else {
            json = {};
        }
...
function saveJson(project, json) {
    // Do some cleanup
    json = mout.object.filter(function (value) {
        // Cleanup empty props, including objects and arrays
        return !mout.lang.isEmpty(value);
    });
...
function setDependencies(project, metadata, answers) {
        ...
        .spread(function (tree, flattened, extraneous) {
            if (extraneous.length) {
                ...
                extraneous.forEach(function (extra) {
                    ...
                    mout.object.mixIn(metadata.dependencies, jsonEndpoint);
                });
            }
